...
def add_predict_args(parser: ArgumentParser) -> ArgumentParser:
    ...
    parser.add_argument(
        ...
        "--preds-path",
        type=Path,
        help="Specify path to which predictions will be saved. Supported formats are CSV, Parquet, Feather, and Pickle. If multiple models are used to make predictions, the average predictions will be saved in the file, and another file ending in '_individual' with the same file extension will save the predictions for each individual model, with the column names being the target names appended with the model index (e.g., '_model_<index>').",
    )
    parser.add_argument(
        "--drop-extra-columns",
        action="store_true",
...
def process_predict_args(args: Namespace) -> Namespace:
    ...
    if args.cal_path is not None and args.cal_path.suffix.lower() not in TABLE_READ_FORMATS:
        raise ArgumentError(
            argument=None,
            message=(
                f"Calibration data must be one of {', '.join(TABLE_READ_FORMATS)}. "
                f"Got {args.cal_path}"
            ),
        )
    if args.output is None:
        default_suffix = (
            args.test_path.suffix if args.test_path.suffix.lower() in TABLE_WRITE_FORMATS else ".csv"
        )
        args.output = args.test_path.parent / (args.test_path.stem + "_preds" + default_suffix)
    if args.output.suffix.lower() not in TABLE_WRITE_FORMATS:
        raise ArgumentError(
            argument=None,
            message=(
                f"Output must be one of {', '.join(TABLE_WRITE_FORMATS)}. "
                f"Got {args.output}"
            ),
        )
    return args
