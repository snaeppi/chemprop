...
def add_train_args(parser: ArgumentParser) -> ArgumentParser:
    ...
    parser.add_argument(
        ...
        "--data-path",
        type=Path,
        help="Path to an input CSV, Parquet, or Feather file containing SMILES and the associated target values",
    )
    parser.add_argument(
        "-o",
        "--output-dir",
    ...
    train_data_args.add_argument(
        "--splits-column",
        help="Name of the column in the input data file containing 'train', 'val', or 'test' for each row.",
    )
    # TODO: Add in v2.1
    # train_data_args.add_argument(
...
def validate_train_args(args):
    ...
    if args.data_path.suffix.lower() not in TABLE_READ_FORMATS:
        raise ArgumentError(
            argument=None,
            message=(
                f"Input data must be one of {', '.join(TABLE_READ_FORMATS)}. "
                f"Got {args.data_path}"
            ),
        )
...
def build_MAB_model(
    ...
    mol_class_weights = None
    if args.task_type in ["classification", "multiclass"]:
        mol_class_weights = prepare_class_weights(
            args, args.mol_target_columns or [], train_dset=train_dset
        )
        if mol_class_weights is not None:
            logger.info(
                f"Using class weights for mol targets: {mol_class_weights.tolist()}"
            )
        ...
        for task_weights, n_tasks, target_cols in zip(
            ...
            criterions.append(
                Factory.build(
                    LossFunctionRegistry[args.loss_function],
                    task_weights=task_weights,
                    class_weights=mol_class_weights
                    if target_cols == args.mol_target_columns
                    else None,
                    v_kl=args.v_kl,
                    eps=args.eps,
                    alpha=args.alpha,
    ...
    for model_idx in range(args.ensemble_size):
        ...
        if test_loader is not None:
            if isinstance(trainer.strategy, DDPStrategy):
                ...
                predss = trainer.predict(model, dataloaders=test_loader, weights_only=False)
            else:
